...
def chave_fonte(s):
    return sem_acento(s).replace("Últimas", "Ultimas").replace("Prévias", "Previas")
...
FONTE_PARA_BOLETINS = {
    ...
    "ANP | Consultas e Audiencias Publicas": [
        "regulatorio-oleo-gas",
        "imobiliario-infraestrutura",
        "ambiental-esg",
    ],
    ...
    "ANTT | Noticias - Defeso Eleitoral": [
        "regulatorio-oleo-gas",
        "imobiliario-infraestrutura",
        "ambiental-esg",
    ],
...
def normalizar_lista(valor, motivo):
    ...
    for x in valor if isinstance(valor, list) else []:
        f = str(x.get("fonte", "")).strip() if isinstance(x, dict) else str(x).strip()
        m = str(x.get("motivo", motivo)).strip() if isinstance(x, dict) else motivo
        if f and f not in vistos:
            vistos.add(f)
            out.append({"fonte": f, "motivo": m})
    return out


def salvar(caminho, dados):
    tmp = caminho + ".tmp"
    with open(tmp, "w", encoding="utf-8") as a:
...
def scrape(fc, url):
    for t in range(1, MAX_TENTATIVAS_SCRAPE + 1):
        try:
            ...
            if not recuperavel(e) or t == MAX_TENTATIVAS_SCRAPE:
                raise
        ...
        for t in range(1, TENTATIVAS_POR_MODELO + 1):
            try:
                ...
                if not isinstance(d, dict) or not isinstance(d.get("itens"), list):
                    raise ValueError("JSON sem lista itens")
                logs.append({"modelo": modelo, "tentativa": t, "status": "sucesso"})
                return (d, modelo, logs)
            except Exception as e:
                logs.append(
                    {
                        "modelo": modelo,
                        "tentativa": t,
                        "status": "erro",
                        "erro": " ".join(str(e).split())[:500],
                    }
                )
                if not recuperavel(e) and (
                    not isinstance(e, (ValueError, json.JSONDecodeError))
                ):
                    return (None, "", logs)
                if t < TENTATIVAS_POR_MODELO:
                    time.sleep(
                        ESPERAS_GEMINI_SEGUNDOS[
                            min(t - 1, len(ESPERAS_GEMINI_SEGUNDOS) - 1)
                        ]
                    )
    return (None, "", logs)
...
def carregar_fontes(inicio, agora, hoje):
    with open(FONTES_PATH, encoding="utf-8") as a:
        fontes = json.load(a)
    ...
    din = [
        {
            ...
            "url": "http://www4.planalto.gov.br/legislacao/portal-legis/resenha-diaria/"
            + meses[hoje.month - 1]
            + "-resenha-diaria",
            "ativo": True,
        },
        {
            ...
            "url": "https://www.bcb.gov.br/estabilidadefinanceira/buscanormas?dataInicioBusca="
            + di
            + "&dataFimBusca="
            + df
            + "&tipoDocumento=Todos",
        ...
        },
        {
            ...
            "url": "https://www.ccee.org.br/busca-ccee?q=&dtIni="
            + di
            + "&dtFim="
            + df
...
def main():
    ...
    inicio = datetime.datetime.combine(
        hoje - datetime.timedelta(days=3 if hoje.weekday() == 0 else 1),
        datetime.time(),
        tzinfo=brt,
    )
    fontes = carregar_fontes(inicio, agora, hoje)
    ativas = []
    suspensas = []
    inativas = []
    for f in fontes:
        if not f.get("ativo", True):
            inativas.append(f)
            continue
        reativar = f.get("reativar_em")
        suspenso = bool(f.get("suspenso")) and (
            not reativar or hoje < datetime.date.fromisoformat(reativar)
        )
        if suspenso:
            suspensas.append(
                {
                    "fonte": f["fonte"],
                    "motivo": f.get("motivo_suspensao", "Suspensão temporária"),
                    "reativar_em": reativar or "",
                }
            )
        else:
            ativas.append(f)
    print(
        f"{len(ativas)} fontes a processar; {len(suspensas)} suspensas; {len(inativas)} inativas"
    )
    with open(PROMPT_PATH, encoding="utf-8") as a:
        prompt = a.read()
    dossier = []
    log = {
        ...
        "janela": {
            ...
            "fim": agora.strftime("%Y-%m-%dT%H:%M"),
        },
        "fontes_processadas": [],
        "fontes_suspensas": suspensas,
        "fontes_inativas": [f["fonte"] for f in inativas],
    }
    fc = Firecrawl(api_key=FIRECRAWL_API_KEY)
    for i, f in enumerate(ativas, 1):
        try:
            r = scrape(fc, f["url"])
            c = (r.markdown or "")[:MAX_CONTEUDO_CHARS]
            erro = (
                ""
                if len(c) >= MIN_CONTEUDO_CHARS
                else f"Conteúdo muito curto ({len(c)} chars)"
            )
            dossier.append(
                {
                    "fonte": f["fonte"],
                    ...
                    "url": f["url"],
                    "tipo_coleta": f.get("tipo_coleta", "pagina"),
                    "conteudo": c if not erro else "",
                    **({"erro_tecnico": erro} if erro else {}),
                }
            )
            log["fontes_processadas"].append(
                {
                    "fonte": f["fonte"],
                    "status": "ok" if not erro else "erro_tecnico",
                    "tamanho_chars": len(c),
                    **({"detalhe": erro} if erro else {}),
                }
            )
        except Exception as e:
            m = " ".join(str(e).split())[:300]
            dossier.append(
                {
                    "fonte": f["fonte"],
                    "categoria": f["categoria"],
            ...
            log["fontes_processadas"].append(
                {"fonte": f["fonte"], "status": "erro", "erro": m}
            )
        if i < len(ativas):
            time.sleep(INTERVALO_ENTRE_SCRAPES_SEGUNDOS)
    cliente = genai.Client(api_key=GEMINI_API_KEY)
    try:
        boletim, modelo, tentativas = gemini(
            cliente,
            prompt
            + "\n\n## Contexto\n"
            + json.dumps(
                {"inicio": log["janela"]["inicio"], "fim": log["janela"]["fim"]},
                ensure_ascii=False,
            )
            + "\n\n## Dossier\n"
    ...
    if not boletim:
        ...
        salvar(LOG_PATH, log)
        raise SystemExit("ERRO: cascata Gemini falhou; boletim anterior preservado.")
    boletim["data_execucao"] = hoje.isoformat()
    boletim["janela_aplicada"] = log["janela"]
    boletim["modelo_gemini_utilizado"] = modelo
    for k, m in [
        ("fontes_sem_resultado", "Página acessada sem conteúdo utilizável."),
        ("fontes_sem_publicacao_hoje", "Nenhuma publicação identificada na janela."),
        ("fontes_com_erro_tecnico", "Erro técnico na coleta."),
    ]:
        boletim[k] = normalizar_lista(boletim.get(k, []), m)
    erros = [
        {"fonte": x["fonte"], "motivo": x["erro_tecnico"]}
        for x in dossier
        if x.get("erro_tecnico")
    ]
    nomes = {x["fonte"] for x in erros}
    boletim["fontes_sem_resultado"] = [
        x for x in boletim["fontes_sem_resultado"] if x["fonte"] not in nomes
    ]
    boletim["fontes_sem_publicacao_hoje"] = [
        x for x in boletim["fontes_sem_publicacao_hoje"] if x["fonte"] not in nomes
    ]
    existentes = {x["fonte"] for x in boletim["fontes_com_erro_tecnico"]}
    boletim["fontes_com_erro_tecnico"] += [
        x for x in erros if x["fonte"] not in existentes
    ]
    itens = []
    descartados = []
    for x in boletim["itens"]:
        ...
        try:
            ok = not ds or inicio.date() <= datetime.date.fromisoformat(ds[:10]) <= hoje
        except ValueError:
            x["data_publicacao"] = ""
            ok = True
    ...
    com_rejeicao = 0
    for x in itens:
        fonte = str(x.get("fonte", ""))
        permitidos = set(FONTE_PARA_BOLETINS.get(chave_fonte(fonte), []))
        conf = x.get("boletins_confirmados", [])
        conf = conf if isinstance(conf, list) else []
        sugeridos = {s for s in conf if s in BOLETINS_DISPONIVEIS}
        finais = [s for s in BOLETINS_DISPONIVEIS if s in permitidos & sugeridos]
        bloqueados = sugeridos - permitidos
        if bloqueados:
            com_bloqueio += 1
        for s in bloqueados:
            bloqueios.setdefault(s, []).append(x.get("titulo", ""))
        rej = (
            [r for r in x.get("boletins_rejeitados", []) if isinstance(r, dict)]
            if isinstance(x.get("boletins_rejeitados", []), list)
            else []
        )
        existentes = {r.get("boletim") for r in rej}
        for s in bloqueados:
            if s not in existentes:
                rej.append(
                    {
                        "boletim": s,
                        "motivo": f"Filtro 1: fonte '{fonte}' não está mapeada para este Radar",
                    }
                )
        x["boletins_rejeitados"] = rej
        x["boletins"] = finais
        pcs = x.get("palavras_chave_detectadas", [])
        x["palavras_chave_detectadas"] = pcs if isinstance(pcs, list) else []
        if rej:
            com_rejeicao += 1
        for r in rej:
            if r.get("boletim"):
    ...
    boletim["boletins_config"] = {
        ...
        "fontes_em_defeso": suspensas,
    }
    stats = {
        s: {
            "nome": NOMES_RADARES[s],
            "clusters": CLUSTERS_POR_BOLETIM[s],
            "total": sum((s in x.get("boletins", []) for x in itens)),
        }
        for s in BOLETINS_DISPONIVEIS
    }
    boletim["estatisticas_por_boletim"] = stats
    boletim["auditoria"] = {
        "total_itens": len(itens),
        "itens_com_alguma_rejeicao": com_rejeicao,
        "itens_com_bloqueio_f1": com_bloqueio,
        "rejeicoes_por_boletim": dict(rejeicoes),
    ...
    log["resultado"] = {
        ...
        "itens_descartados_pos_validacao": len(descartados),
        "fontes_ativas": len(ativas),
        "fontes_suspensas": len(suspensas),
        "fontes_inativas": len(inativas),
        ...
        "filtro1_bloqueios": {s: len(v) for s, v in bloqueios.items()},
        "auditoria": boletim["auditoria"],
    }
    log["filtro1_bloqueios_detalhe"] = bloqueios
    salvar(OUTPUT_PATH, boletim)
    salvar(LOG_PATH, log)
    print("Concluído:", len(itens), "itens; modelo", modelo)
