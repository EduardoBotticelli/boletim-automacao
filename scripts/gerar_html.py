...
def render_item(item):
    ...
    html = '<div style="background:#fff;border-left:4px solid #1a4d2e;padding:14px 18px;margin:10px 0;border-radius:4px;box-shadow:0 1px 2px rgba(0,0,0,0.06);">'
    html += '<h3 style="margin:0 0 6px 0;font-size:14.5px;color:#1a1a1a;font-weight:600;line-height:1.4;">' + titulo + '</h3>'
    html += '<p style="margin:4px 0 8px 0;font-size:13px;color:#4a4a4a;line-height:1.5;">' + resumo + '</p>'
    html += '<div style="font-size:12px;color:#7a7a7a;border-top:1px solid #eee;padding-top:6px;">'
    html += '<span style="margin-right:14px;"><strong>Publicado:</strong> ' + data_pub + '</span>'
    html += '<a href="' + url + 'l += '</div>'
    html += '<div style="font-size:11px;color:#999;margin-top:4px;font-style:italic;">Motivo da filtragem: ' + motivo + '</div>'
    html += '</div>'
    return html
...
def gerar_email_html(slug, nome_boletim, itens, fontes_desta_area, aviso_email_pendente):
    ...
    parts.append('<p style="margin:0 0 10px 0;"><strong>Fe, boa tarde.</strong></p>')
    ...
    # FONTES CONFIGURADAS
    fontes_str = escape_html(", ".join(fontes_desta_area) if fontes_desta_area else "Nenhuma")
    ...
    parts.append(fontes_str)
    parts.append('</div>')

    # AVISO DE FONTES POR EMAIL
    if aviso_email_pendente:
        ...
        parts.append('<strong>Aviso:</strong> As seguintes fontes desta area chegam por e-mail e ainda nao estao integradas ao sistema (integracao futura): ' + escape_html(", ".join(aviso_email_pendente)) + '.')
        parts.append('</div>')
    ...
    parts.append('<p style="margin:0;">Sistema em fase de validacao. Feedback sao muito bem-vindos e serao usados para calibrar o filtro.</p>')
    parts.append('</div>')
