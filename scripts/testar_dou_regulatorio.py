...
OUT = BASE / "output"
OJ = OUT / "teste_dou_regulatorio.json"
OH = OUT / "teste_dou_regulatorio.html"
...
def main():
    if not FK or not GK:
        raise SystemExit("ERRO: secrets ausentes.")
    OUT.mkdir(exist_ok=True)
    data = os.environ.get("DOU_DATA") or datetime.datetime.now(
        ZoneInfo("America/Sao_Paulo")
    ).strftime("%d-%m-%Y")
    datetime.datetime.strptime(data, "%d-%m-%Y")
    secao = os.environ.get("DOU_SECAO", "do1")
    url = (
        os.environ.get("DOU_URL")
        or f"https://www.in.gov.br/leiturajornal?data={data}&secao={secao}"
    )
    fc = Firecrawl(api_key=FK)
    conteudo = ""
    for t in range(1, 4):
        try:
            conteudo = (
                fc.scrape(url, formats=["markdown"], only_main_content=True).markdown
                or ""
            )[:120000]
            break
            ...
            time.sleep(65)
    if not conteudo:
        raise SystemExit("ERRO: DOU sem conteúdo.")
        ...
        for m in MODELOS:
            for t in range(1, 3):
                try:
                    ...
                    if not isinstance(d.get("publicacoes"), list):
                        raise ValueError("publicacoes ausente")
                    dados = d
                    modelo = m
                    tent.append({"modelo": m, "tentativa": t, "status": "sucesso"})
                    break
                except Exception as e:
                    tent.append(
                        {
    ...
    if not dados:
        salvar(
            OJ,
            {
                ...
                "data_dou": data,
                "url_edicao": url,
                "tentativas_gemini": tent,
            },
        )
        raise SystemExit("ERRO: Gemini falhou.")
    pubs = []
    for x in dados["publicacoes"]:
        ...
        y["palavras_chave"] = [
            str(p).strip()
            for p in x.get("palavras_chave", [])
            if isinstance(p, str) and p.strip()
        ]
        if y["titulo"] and y["resumo"]:
            pubs.append(y)
    dados.update(
        {
            "publicacoes": pubs,
            "data_dou": data,
            "secao": "Seção 1" if secao == "do1" else secao,
            "url_edicao": url,
            "modelo_gemini_utilizado": modelo,
            "tentativas_gemini": tent,
            "total_publicacoes_regulatorias": len(pubs),
        }
    )
    salvar(OJ, dados)
    cards = (
        "".join(
            (
                f"<article><small>{html.escape(x['orgao'])} · {html.escape(x['tipo_ato'])}</small><h2>{html.escape(x['titulo'])}</h2><p>{html.escape(x['resumo'])}</p><p><b>Aderência:</b> {html.escape(x['justificativa_regulatorio'])}</p></article>"
                for x in pubs
            )
        )
        or "<p>Nenhuma publicação aderente identificada.</p>"
    )
    doc = f'<!doctype html><html lang="pt-BR"><head><meta charset="utf-8"><title>Teste DOU Regulatório</title><style>body{{background:#f0f2f1;font-family:Arial}}main{{max-width:900px;margin:24px auto}}header{{background:#0d3320;color:white;padding:28px}}article{{background:white;padding:20px;margin-top:14px}}h2{{color:#0d3320}}</style></head><body><main><header><h1>DOU · Cluster Regulatório</h1><p>{html.escape(data)} · {len(pubs)} publicações</p></header>{cards}</main></body></html>'
    tmp = OH.with_suffix(".html.tmp")
    tmp.write_text(doc, encoding="utf-8")
    os.replace(tmp, OH)
    print("Teste DOU:", len(pubs), "publicações")
