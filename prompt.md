...
a descrição oficial da área;
as palavras-chave indicativas;
o contexto integral da publicação;
o órgão responsável;
o efeito jurídico, regulatório ou empresarial da publicação.
...
Todos os slugs pertencem à lista permitida.
As confirmações têm justificativa temática.
Classificação múltipla é permitida.
Nenhum item foi classificado somente pela fonte.
Nenhuma data foi inventada.
Nenhuma informação externa foi adicionada.
Não há duplicidades evidentes.
Todos os itens têm título, fonte, resumo e motivo.
Todos os arrays obrigatórios existem.
A saída é JSON válido e não tem texto externo.
Nenhum Radar aparece simultaneamente como confirmado e rejeitado.
Fontes com erro técnico não aparecem em outra categoria de fonte.
Os nomes das fontes correspondem exatamente ao dossier.
Todas as URLs vieram do dossier.
Itens sem classificação permanecem disponíveis para revisão humana.
